	...
	survey <- surv1 |> 
	  ...
	  mutate(
		...
		labelled::set_variable_labels(
			...
			, state.fct                    = "State (factor)"
			, demog_data.fct               = "How obtained demo proportions (factor)"
			, inpatientcare.fct            = "Inpatient Care (factor)"
			, teamlearn_how_train.fct      ="Training Method (factor)"
			, train_together.fct           ="Train as practice team (factor)"
			, train_small_teams.fct        ="Train in smaller teams (factor)"
			, train_idv.fct                ="Train everyone individually (factor)"
		)  |>
		...
		select(recordid,
					 ...
					 starts_with("state"),
					 survey_dtd,
					 starts_with("consulted"),
					 starts_with("num_"),
					 starts_with("fte_"),
					 starts_with("patient_"),
					 starts_with("inpatientcare"),
					 starts_with("race"),
					 hispanic,
					 non_hispanic,
					 hispanic_unknown,
					 ...
					 starts_with("teamlearn"),
					 starts_with("train"),)
	
	saveRDS(survey, here::here("data/survey.RDS"))
	write.csv(survey, here::here("data/survey.csv"))
