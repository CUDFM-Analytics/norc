...
PROC SQL; 
CREATE TABLE survey_prac_check AS 
SELECT a.prac_entity_split_id as inclusion_list
     , b.practice_id as survey_bl_ids
     , CASE WHEN a.prac_entity_split_id = b.practice_id THEN '1' ELSE '0' END AS Match
FROM out.inclusion as A FULL JOIN out.survey_baseline as B
ON   a.prac_entity_split_id = b.practice_id
/*have 0's at top */
ORDER BY Match
; 
QUIT; 
...
%put List of Variables=%mf_getvarlist(out.survey_baseline);

* Run macro for univariate data (globals);
%summary(ds=out.survey_baseline,out=out.summary_survey);
